...
async def test_resource_validators(tmp_path, jp_fetch, jp_server_config):
    ...
    resources = await cc.set_resources([
        ...
        {'name': 'invalid-1', 'url': f"osfs://{tmp_path.as_posix()}/test-invalid-A"},
        {'name': 'invalid-2', 'url': f"osfs://{tmp_path.as_posix()}/test-invalid-B"},
        {'name': 'invalid-3', 'url': f"osfs://{tmp_path.as_posix()}/invalid-C"},
        {'name': 'invalid-4', 'url': f"osfs://{tmp_path.as_posix()}/foo"},
        {'name': 'invalid-5', 'url': f"osfs://{tmp_path.as_posix()}/test-valid-A/non-existant"},
        {'name': 'invalid-6', 'url': f"non-existant://{tmp_path.as_posix()}/test-valid-B"},
    ])
    names = set(map(lambda r: r['name'], resources))
