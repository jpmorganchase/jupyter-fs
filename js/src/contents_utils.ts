 ...
 */
export async function exposePath<T extends IContentRow>(contents: ContentsModel<T>, path: string[]): Promise<Content<T>> {
 ...
 * @param path The path to expose, relative to the root (i.e. first entry matches path of root)
 */
export async function exposeAndSelectPath<T extends IContentRow>(contents: ContentsModel<T>, path: string[], add?: boolean): Promise<Content<T>> {
  const node = await exposePath(contents, path);
